...
const PricingPr = () => {
    ...
    const my_prompt = localStorage.getItem("my_prompt")
    ...
    const handleSubmit = async (planData) => {
        if(["1", "2", "3", "4", "5", "6"].includes(my_plan) && (my_prompt > 0 || my_prompt == -99)){
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (errorData.code === 403) {
                    // console.error('Error 403: Plan update required, contact customer support');
                    setShowUpdatePlanPopUp(true)
                    // toast({
                    //     title: "Error",
                    //     description: "Contact customer support to update your plan!",
                    //     variant: "destructive",
                    // });
                    return; // Exit early if this specific error occurs
                }

                // General error toast for any other non-200 response
                toast({
                    title: "Error",
            ...
            if (!data.ok) {
                console.error('Data indicates an error:', data);
                toast({
                    title: "Error",
    ...
    return (
        <>
            ...
            {showUpdatePlanPopUp && <Dialog open={showUpdatePlanPopUp} onOpenChange={setShowUpdatePlanPopUp}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Turnit Human</DialogTitle>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-1 items-center gap-4">
                           <p>Please contact to customer support to upgrade your plan.</p>
                           <p>Thank you.</p>
                        </div>
                    </div>
                </DialogContent>
