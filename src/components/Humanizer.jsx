...
const Humanizer = ({ fetchData, updatePrompt }) => {
  const [cloudKey, setCloudKey] = useState("")
  const [plan_id, setPlan_id] = useState("");
  const [myPrompt, setMyPrompt] = useState(localStorage.getItem("my_prompt") || 0) // prompt to show
  ...
  const fetchHisrotyData = async () => {
    try {
      ...
      if (createdId && title) {
        ...
        // handleSaveContent(filteredData[0]?.humanizedContent)
        setIsSecondContainerVisible(true);
  ...
  const fetchRewriteData = async (rephrase) => {
    if((plan_id === "3" || plan_id === "6") && cloudKey == ""){
      toast({
        title: 'Error',
        description: 'Please add Cloud-Key to access the tool.',
        variant: "destructive",
      });
      return
    }
    ...
    const body = {
      prompt: text,
      // expected_word_count: wordCount,
      tone: "Professional",
      mode: "Medium",
      business: true,
      rephrase: rephrase
    }
    if (cloudKey && (plan_id === "3" || plan_id === "6")) {
      body["user_api_key"] = cloudKey;
    }
    try {
      const response = await fetch("https://zf6tsnfqdg.us-east-1.awsapprunner.com/stealthify", {
        method: "POST",
        headers,
      ...
      setHumanizedData(result.result);
      handleSaveContent(result.result)
      toast({
        title: "Success",
  ...
  const handleSaveContent = async (humanizedData) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMyPrompt(result?.data?.prompt);
        updatePrompt(result?.data?.prompt)
        localStorage.setItem("my_prompt", result?.data?.prompt || 0)
  ...
  const fetchCloudKey = async (userId, planId) => {
    ...
    <div className={styles.turnitGencontainer}>
      ...
      <div className={styles.turnitGencontainerFirst}>
        <div className={`${styles.turnitGenPaste} ${styles.turnitGenPastepadi}`}>
          <div>
            <div className={`cursor-pointer ${styles.turnitGenPastepadiback}`} onClick={handleBack}>
          ...
          <form onSubmit={handelSubmit}>
            ...
            <div className={styles.turnitGenCounter}>
              ...
              {(plan_id == -2 || plan_id == -1) ? (
                ...
                <button type="submit" className={`cursor-pointer ${styles.turnitgenerateButton}`}>
                  Humanize
          ...
          <div className='turnitHumanizerPercentleft'>
            ...
            <div className='turnithumanizehighlightcount'>
              ...
              {(plan_id == -2 || plan_id == -1) ? (
                ...
                <button className="cursor-pointer" onClick={handleRegenerateClick}>
                  Regenerate
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path
